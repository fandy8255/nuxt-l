    ...
    <div class="card border-1 my-2 shadow-sm">
        <div class="card-img-top m-0 overflow-hidden bsb-overlay-hover text-center" style="max-height: 40vh !important">
            <NuxtLink :to="'/tienda/' + prodId">
                ...
                <NuxtImg provider="bunny" :src="imgSrc" width="300px" :quality="50"
        ...
        <div class="card-body bg-white p-1 mx-1 border-top">
            <div class="d-flex">
                <div class="row w-100">
                    <div class="col-6">
                        <UserImgComponent :image="usernamePicture" :username="username" />
                    </div>
                    <div class="col-6 text-end" >
                        <div class="lk d-flex justify-content-end align-items-end">
                            <LikeButton :likedProductId="prodId" :productOwnerId="productOwner" />
                            <div>{{ likeCount }}</div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-xl-1 p-sm-1">
                ...
                <p>{{ productDescription.length < 150 ? productDescription : productDescription.slice(0, 150) + '...'
                        }}</p>
                        ...
                        <div class="d-flex justify-content-between flex-lg-wrap">
                            ...
                            <DeleteProductModal :product="product" :productId="prodId"
                                v-if="userStore.username === username || isAd" @updateProductsStore="updateProducts"
                                @click="test" />
                        </div>
            </div>
        </div>
...
const { likeCount, productOwner, username, usernamePicture, imgSrc, productTitle, productDescription, prodId, productCategory, productPrice, isAd } =
    defineProps(['likeCount', 'productOwner', 'username', 'usernamePicture', 'imgSrc', 'productTitle', 'productDescription', 'prodId', 'productCategory', 'productPrice', 'isAd'])
